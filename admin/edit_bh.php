...
<div class='row'>
    <div class="col-lg-12 col-md-12 col-xs-12 col-sm-12">
            ...
            if($_SERVER['REQUEST_METHOD']=='POST')
            {
                ...
                if(empty($error)){
					...
					else
					{
                        ...
                        }$sql="SELECT anh,anh_thumb FROM tblbaihat WHERE id={$id}";
                    ...
                    if ($_FILES['mp3']['size']=='') 
					{
						$link_song=$_POST['mp3_hi'];
                    }
                    else{
                        if ($_FILES['mp3']['size']>5000000) 
						{
							$message="Kích thước phải nhỏ hơn 5MB";						
                        }
                        else{	
                    ...
                    $sql1="SELECT noidung FROM tblbaihat WHERE id={$id}";
						$query_a=mysqli_query($dbc,$sql1);
						$mp3=mysqli_fetch_assoc($query_a);
						unlink('../'.$mp3['noidung']);
                }
                 
                $query="UPDATE tblbaihat SET BaiHat='{$baihat}',ChuDe='{$chude}', NgheSi='{$nghesi}', noidung='{$link_song}',
                anh='{$link_img}',
        ...
        $query_id="SELECT BaiHat,ChuDe,NgheSi,noidung,anh,anh_thumb,ordernum,Status FROM tblbaihat WHERE id={$id};";
        $result_id=mysqli_query($dbc,$query_id);
        kt_query($result_id,$query_id);
        if(mysqli_num_rows($result_id)==1){
            list($baihat,$chude,$nghesi,$noidung,$anh,$anh_thumb,$ordernum,$status)=mysqli_fetch_array($result_id,MYSQLI_NUM);
        }
        else{
            $messegers="<p class='required'>ID Bài hát không tồn tại</p>";
        }
        ?>
        ...
        <?php
            ...
            ?>
            <h3>Sửa Bài hát: <?php if(isset($baihat)){ echo $baihat;} ?></h3>
            <div class="form-group">
                <label>Bài hát</label>
                <input type="text" name="baihat" value="<?php if(isset($baihat)){ echo $baihat;} ?>" class="form-control" placeholder="Bài hát">
                <?php
            ...
            <div class="form-group">
                ...
                <input type="hidden" name="mp3_hi" value="<?php echo $noidung; ?>">
